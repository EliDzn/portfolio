  ...
  return (
    <div className="lg:hidden">
      <button
        ...
        onClick={toggleMenu}
        className={[
          "relative z-30 flex size-9 items-center justify-center hover:text-accent hover:cursor-pointer transition-colors duration-150 ease-out",
                ...
                {links.map((link) => (
                  <li key={link.href} className="">
                    <Link
                      ...
                      onClick={closeMenu}
                      tabIndex={isOpen ? 0 : -1}
                      className="block text-h1-mobile font-medium uppercase transition-colors hover:text-accent-hover pr-2"
                    >
                      {link.label}
                    </Link>
                  </li>
